...
namespace Midterm_EquipmentRental_Team2.Repositories
{
    public class RentalRepository : IRentalRepository
    {
        ...
        public IEnumerable<Rental> GetByEquipmentId(int equipmentId) =>
            _context.Rentals.Where(r => r.EquipmentId == equipmentId).ToList();
        public IEnumerable<Rental> GetByCustomerId(int customerId) =>
            _context.Rentals.Where(r => r.CustomerId == customerId).ToList();
        public IEnumerable<Rental> GetActiveByCustomerId(int customerId) =>
            _context.Rentals.Where(r => r.CustomerId == customerId && r.ReturnedAt == null).ToList();
        public IEnumerable<Rental> GetCompletedByCustomerId(int customerId) =>
            _context.Rentals.Where(r => r.CustomerId == customerId && r.ReturnedAt != null).ToList();
        public IEnumerable<Rental> GetOverdue() =>
            _context.Rentals.Where(r => r.DueDate < DateTime.UtcNow && r.ReturnedAt == null).ToList();
        public void Add(Rental rental) { _context.Rentals.Add(rental); _context.SaveChanges(); }
        public void Update(Rental rental) { _context.Rentals.Update(rental); _context.SaveChanges(); }
        public void Delete(Rental rental) { _context.Rentals.Remove(rental); _context.SaveChanges(); }
    }
}
